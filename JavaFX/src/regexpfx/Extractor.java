...
public class Extractor {
  ...
  /** 
   * Función adaptada a RegexpFX que usa Pattern y Matcher para buscar 
   ...
   * @throws IOException
   */
  static String buscarRegexp(String texto, String patron) throws IOException {
    String lineaActual = "";
    Pattern elPatron = Pattern.compile(patron);
    BufferedReader txt = new BufferedReader(new StringReader(texto));
    String coincidencias = "";
    lineaActual = txt.readLine();
    
    while (lineaActual != null) { // Mientras no llegue al EOF...
      Matcher matcher = elPatron.matcher(lineaActual);
      while (matcher.find()) {
        coincidencias += matcher.group();
      }
      lineaActual = txt.readLine();
    } 
